...
import { tandaiLunas, tandaiBelumLunas } from '@/lib/actions'
import { useRouter } from 'next/navigation'

export default function TandaiLunasButtons({ paymentId }: { paymentId: string }) {
  ...
  async function tandai(metode: string) {
    setLoading(true)
    setShowMetode(false)
    await tandaiLunas(paymentId, metode)
    setLoading(false)
    router.refresh()
  }

  if (loading) return <span style={{ fontSize:11, color:'var(--text3)' }}>Menyimpan...</span>
